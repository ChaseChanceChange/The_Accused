...
import { Plus, LayoutGrid, Heart, Sparkles, Info, LogOut, Loader2, AlertTriangle, Scroll, Coffee } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const verifyDiscordToken = async (accessToken: string) => {
    ...
    try {
        ...
        if (guildsRes.ok) {
            ...
            console.warn("Could not fetch guilds. Defaulting to Guest mode.");
        }
        ...
        // LOGIN SUCCESSFUL - Save and Set
        setUser(appUser);
        localStorage.setItem('mystic_user', JSON.stringify(appUser));
        
        if (isMember) {
             showNotification(`Welcome, Member of The Accused!`);
        } else {
  ...
  useEffect(() => {
    ...
    // Supports both #access_token=... and ?access_token=... (just in case)
  ...
  const handleSave = (newEnchantment: Enchantment) => {
    if (user) {
        ...
        showNotification("Please login to forge items.", 'error');
    }
  };
  ...
  const handleDelete = (id: string) => {
      if (user?.isMember) {
        ...
        showNotification("Enchantment Deleted", 'info');
          ...
          showNotification("Access Denied: Moderators Only", 'error');
      }
  };
  ...
  // 2. Not Logged In - Strict Gate (But allows login)
  if (!user) {
      return (
                ...
                <div className="flex items-center gap-3">
                    <div className="text-right hidden sm:block">
                        ...
                        {user.isMember ? (
                            ...
                            <div className="text-[10px] text-gray-400 uppercase tracking-wide flex items-center justify-end gap-1">Guest</div>
                        )}
                    </div>
                    <div className="relative group">
                            <img src={user.avatar} alt="User" className={`w-10 h-10 rounded-full border-2 cursor-pointer ${user.isMember ? 'border-green-500' : 'border-gray-500'}`} />
      ...
      <main className="flex-1 relative">
         ...
         {view === 'create' ? (
             <CreateView 
                onSave={handleSave} 
                user={user} 
                onLoginRequest={() => {}} // User is already logged in if viewing this
             />
         ) : view === 'community' ? (
             <CommunityView />
         ) : (
